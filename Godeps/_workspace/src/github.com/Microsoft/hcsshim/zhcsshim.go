...
var (
	...
	procHcsStartDNSServer                  = modvmcompute.NewProc("HcsStartDNSServer")
	procHcsStopDNSServer                   = modvmcompute.NewProc("HcsStopDNSServer")
...
func hcsStartDNSServer(id string, ioPipe *syscall.Handle) (hr error) {
	var _p0 *uint16
	_p0, hr = syscall.UTF16PtrFromString(id)
	if hr != nil {
		return
	}
	return _hcsStartDNSServer(_p0, ioPipe)
}

func _hcsStartDNSServer(id *uint16, ioPipe *syscall.Handle) (hr error) {
	if hr = procHcsStartDNSServer.Find(); hr != nil {
		return
	}
	r0, _, _ := syscall.Syscall(procHcsStartDNSServer.Addr(), 2, uintptr(unsafe.Pointer(id)), uintptr(unsafe.Pointer(ioPipe)), 0)
	if int32(r0) < 0 {
		hr = syscall.Errno(win32FromHresult(r0))
	}
	return
...
func hcsStopDNSServer(id string) (hr error) {
	var _p0 *uint16
	_p0, hr = syscall.UTF16PtrFromString(id)
	if hr != nil {
		return
	}
	return _hcsStopDNSServer(_p0)
}

func _hcsStopDNSServer(id *uint16) (hr error) {
	if hr = procHcsStopDNSServer.Find(); hr != nil {
		return
	}
	r0, _, _ := syscall.Syscall(procHcsStopDNSServer.Addr(), 1, uintptr(unsafe.Pointer(id)), 0, 0)
	if int32(r0) < 0 {
		hr = syscall.Errno(win32FromHresult(r0))
	}
	return
