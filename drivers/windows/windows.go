	...
	if config.HnsID == "" {
		...
		if nInfo != nil {
			...
			for _, subnet := range hnsresponse.Subnets {
				found = false
				for _, ipData := range ipV4Data {
					if subnet.AddressPrefix == ipData.Pool.String() &&
						subnet.GatewayAddress == ipData.Gateway.IP.String() {
						found = true
						break
					}
			...
			if !found {
				for _, subnet := range hnsresponse.Subnets {
					ipamV4Conf := driverapi.IpamConf{}
					ipamV4Conf.PreferredPool = subnet.AddressPrefix
					ipamV4Conf.Gateway = subnet.GatewayAddress
					v4Conf = append(v4Conf, &ipamV4Conf)
				}

				logrus.Debugf("Overriding subnet config for network ")

				if err := nInfo.IpamOverrideConfig(4, v4Conf); err != nil {
					return err
				}
			}
		}
